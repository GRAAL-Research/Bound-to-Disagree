...
def kl_inv(q, epsilon, mode, tol=10**-9, nb_iter_max=1000):
    ...
    """
    if torch.isnan(torch.tensor(q)):
        return 1.0
    if q > 1:
        return 1.0
    assert mode == "MIN" or mode == "MAX"
    assert isinstance(q, float) and q >= 0 and q <= 1, f"Value of q : {q}"
    assert isinstance(epsilon, float) and epsilon > 0.0, f"Value of epsilon : {epsilon}"
